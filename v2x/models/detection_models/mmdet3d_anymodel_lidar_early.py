...
class EarlyFusion(BaseModel):
    ...
    def forward(self, vic_frame, filt, prev_inf_frame_func=None, *args):
        ...
        Inf_points = read_pcd(
            osp.join(vic_frame.path, "infrastructure-side", vic_frame.inf_frame["pointcloud_path"]))  # read inf points
        Veh_points = read_pcd(
            osp.join(vic_frame.path, "vehicle-side", vic_frame.veh_frame["pointcloud_path"]))  # read veh points
        ...
        for i in range(len(Inf_points.pc_data)):
            temp = get_trans.single_point_transformation(
